...
std::string ethash_cu_miner::platform_info(unsigned _deviceId)
{
	...
	if (cudaDriverGetVersion(&driver_version) == cudaErrorInvalidValue)
	{
		cout << cudaGetErrorString(cudaErrorInvalidValue) << endl;
		return std::string();
	}
	...
	char platform[10];
	sprintf(platform, "%d %d", runtime_version, driver_version);

	char compute[5];
	sprintf(compute, "%d.%d", device_props.major, device_props.minor);

	return "{ \"platform\": \"" + std::string(platform) + "\", \"device\": \"" + device_props.name + "\", \"version\": \"Compute " + std::string(compute) + "\" }";
}
...
bool ethash_cu_miner::init(ethash_params const& params, std::function<void(void*)> _fillDAG, unsigned workgroup_size, unsigned _deviceId)
{
	// store params
	m_params = params;
	...
	// patch source code
	std::string code(ethash_cu_miner_KERNEL, ethash_cu_miner_KERNEL + ethash_cu_miner_KERNEL_SIZE);
	GROUP_SIZE = m_workgroup_size;
	DAG_SIZE = (unsigned)(params.full_size / ETHASH_MIX_BYTES);
	ACCESSES = ETHASH_ACCESSES;
	MAX_OUTPUTS = c_max_search_results;
